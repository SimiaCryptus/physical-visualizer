...
export class Gestures {
  constructor(el, signals, events) {
    ...
    el.addEventListener('pointerdown', (e) => {
      ...
      if (e.isPrimary) {
        ...
        this._pressTimer = setTimeout(() => {
          if (this._down && this.pointers.size === 1) events.emit('gesture:longpress');
        }, 600);
      }
    ...
    el.addEventListener('pointermove', (e) => {
      ...
      if (e.isPrimary) {
        this._setPos(e);
        if (this._start && Math.hypot(e.clientX - this._start.x, e.clientY - this._start.y) > 12)
          clearTimeout(this._pressTimer);
      }
    ...
    const up = (e) => {
      ...
      const dx = e.clientX - s.x,
        dy = e.clientY - s.y,
        dt = performance.now() - s.t;
      if (dt < 350 && Math.abs(dx) > 60 && Math.abs(dx) > Math.abs(dy))
        events.emit('gesture:swipe', dx < 0 ? 'left' : 'right');
      else if (dt < 350 && dy < -80 && Math.abs(dy) > Math.abs(dx))
        events.emit('gesture:swipe', 'up');
      else if (dt < 250 && Math.hypot(dx, dy) < 10) {
        const now = performance.now();
        if (now - this._lastTap < 300) {
          events.emit('gesture:doubletap');
          this._lastTap = 0;
