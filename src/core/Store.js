...
const DB = 'physviz',
  VER = 1,
  STORES = ['prefs', 'tracks', 'skins', 'presets'];
...
export class Store {
  ...
  put(store, key, val) {
    return this._tx(store, 'readwrite', (s) => s.put(val, key));
  }
  del(store, key) {
  ...
  estimate() {
    return navigator.storage?.estimate?.() ?? Promise.resolve({ usage: 0, quota: Infinity });
  }
}
