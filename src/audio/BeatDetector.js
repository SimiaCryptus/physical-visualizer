...
export class BeatDetector {
  constructor(
    signals,
    bins,
    { sensitivity = 1.5, refractory = 0.12, medianLen = 43, ring = 256 } = {}
  ) {
    this.S = signals;
    this.prev = new Uint8Array(bins);
    this.sens = sensitivity;
    this.refractory = refractory;
    this.fluxHist = new Float32Array(medianLen);
    this.fluxHead = 0;
  ...
  update(freq, dt) {
    ...
    this.ring[this.ringHead] = flux;
    this.ringHead = (this.ringHead + 1) % this.ring.length;
    this.sorted.set(this.fluxHist);
    this.sorted.sort();
    ...
    if (this.time - this.lastBpmCalc > 1) {
      this.lastBpmCalc = this.time;
      this._estimateBpm();
    }
